...
class ZipReplace:
    ...
    def process(self, zipprocessor):
        for filename in os.listdir(zipprocessor.temp_directory):
            with open(zipprocessor._full_filename(filename)) as infile, \
                open(zipprocessor._full_filename(filename), 'w') as outfile:
                outfile.write(infile.read().replace(self.search_string,
                                                    self.replace_string))
